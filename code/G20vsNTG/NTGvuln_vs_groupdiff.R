...
rm(list=setdiff(ls(),c('params','goi','probe','injection.site')))
basedir <- params$basedir
setwd(basedir)
diffmodel.savedir <- paste(params$opdir,'diffmodel/',paste0(injection.site,collapse='-'),'/',sep='')
savedir <- paste(params$opdir,'G20vsNTG/path_vs_vuln/',paste0(injection.site,collapse='-'),'/',sep='')
...
NTG.vuln <- read.csv(file=paste0(diffmodel.savedir,'NTGvulnerability.csv'),check.names = F)
NTG.vuln.hemi <- t(read.csv(file=paste0(diffmodel.savedir,'NTGvulnerability_hemiaverage.csv'),row.names = 1))[1,]
...
p.list <- lapply(1:length(tps), function(t.) p.xy(y = log(Grp.mean.G20[[t.]]/Grp.mean.NTG[[t.]],base=10),x=NTG.vuln[,paste(tps[t.],'MPI')],xlab = 'NTG Vulnerability',
                                       ylab = 'log(G2019S/NTG)\nPathology',ttl = paste(tps[t.],'MPI'),col = plot.col,alpha=0.8))
p.all <- plot_grid(plotlist = p.list,nrow=1)
ggsave(p.all,filename = paste(savedir,'G2019-NTGvsNTGVulnerability.pdf',sep=''),
...
G10.NTG.hemiaverage <- lapply(1:length(tps), function(t.) log(hemi.average(Grp.mean.G20[[t.]],TRUE)/hemi.average(Grp.mean.NTG[[t.]],TRUE),base=10))
p.list <- lapply(1:length(tps), function(t.) p.xy(y = G10.NTG.hemiaverage[[t.]],x=NTG.vuln.hemi,xlab = 'NTG Vulnerability',
                                                  ylab = 'log(G2019S/NTG)\nPathology',ttl = paste(tps[t.],'MPI'),col = plot.col,alpha=0.8))
p.all <- plot_grid(plotlist = p.list,nrow=1)
ggsave(p.all,filename = paste(savedir,'G2019-NTGvsNTGVulnerability_HemiAverage.pdf',sep=''),
       units = 'cm',height = 3.75,width = 3.75*length(tps))
