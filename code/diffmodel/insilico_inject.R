...
setwd(basedir)
savedir <- paste(params$opdir,'insilico_injections/',sep='')
...
datadir <- paste(params$opdir,'diffmodel/bidirectional/',paste0(params$injection.site,collapse='-'),'/',sep='')
load(file=paste0(datadir,grp,'CNDRSpaceBidirectionalFit_data.RData'))
...
df <- lapply(1:length(params$tps), function(t) data.frame(pred.retro = Xt.Grp.retro[,t,drop=FALSE], pred.antero = Xt.Grp.antero[,t,drop=FALSE]))

insilico.pred <- matrix(NA,nrow=n.regions.CNDR,ncol=length(params$tps),dimnames = list(path.names,paste(tps,'MPI')))
m.pred <- lapply(1:length(tps), function(t.) predict(m[[t.]],df[[t.]]))
