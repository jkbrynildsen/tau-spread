...
load(paste0(opdir,'processed/pathdata.RData')) # load path data and ROI names
load(file = paste0(savedir,'AlternateSeedFits_OneLM.RData'))
load(file = paste0(params$opdir,'diffmodel/bidirectional_onelm/',paste0(injection.site,collapse='-'),'/all_mice_BidirectionalOptimOneLM_data.RData'))
...
ggsave(p.null.seeds,filename=paste0(savedir,'SeedSpecificity_OneLM.pdf'),
	width=7,height=5,units='cm')

# relate alternate fit to connectivity and distance
W <- read.csv(paste0(params$opdir,'processed/W.csv'), header = FALSE) # subbed this and the below line for the above line
W <- as.matrix(W)
...
ggsave(p,filename=paste0(savedir,'RandomSeedFitsVsConnectivity_OneLM.pdf'),
