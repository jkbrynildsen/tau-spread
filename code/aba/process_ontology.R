...
unique.identifier <- 'jKbRyn' # use any relatively unique string just to make sure you extract the right list elements in next step
...
structure_ids <- as.numeric(ont.un[grep(paste0(".id",unique.identifier),names(ont.un))])
names(structure_ids) <- names(ont.un[grep(paste0(".id",unique.identifier),names(ont.un))])
...
path.names.id.key <- ontology.get.name.id.key(path.names,df.key)
region.names.id.key <- ontology.get.name.id.key(region.names,df.key)
# check for regions that weren't able to be labeled
unlist(path.names.id.key)[is.na(unlist(path.names.id.key))]
unlist(region.names.id.key)[is.na(unlist(region.names.id.key))]

save(path.names.id.key,region.names.id.key,file=paste0(abadir,'ontologies/keys.RData'))
