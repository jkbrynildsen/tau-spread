...
for(mdl.name in mdl.names){
  ...
  if(mdl.name == 'Euclidean'){ctrl$Xo <- get.Xo(region.names.mdl,injection.site)} # exclude regions with missing coordinate data
  else {ctrl$Xo <- get.Xo(region.names,injection.site)}
